                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC)
{
    ...
    // TTC = -dT / (1 - meanDistRatio); // result 13

    // TODO: STUDENT TASK (replacement for meanDistRatio)(built by me)
    /*
    double medianDistRatio;
    for(int i =0; i<distRatios.size()/2; i++)
    {
        medianDistRatio = distRatios[i];
    }
    TTC = -dT / (1 - medianDistRatio); // result 11
    */

    // SOLUTION
    std::sort(distRatios.begin(),distRatios.end());
    long meindex =floor(distRatios.size()/2);
    double meDistRatio = distRatios.size() % 2 == 0 ? (distRatios[meindex] + distRatios[meindex-1] )/2 : distRatios[meindex];
    TTC = -dT / (1 - meDistRatio); // 12
